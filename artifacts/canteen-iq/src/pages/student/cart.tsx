...
export default function StudentCart() {
  ...
  const handlePlaceOrder = () => {
    ...
    createOrder.mutate(
      ...
      },
      {
        onSuccess: (data) => {
          clearCart();
          toast({ title: "Order Placed", description: `Token #${data.tokenNumber}` });
  ...
  const generateTimeSlots = () => {
    const slots = [];
    const now = new Date();
    let current = new Date(now.getTime() + 15 * 60000); // Next 15 mins
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto pb-20">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Your Cart</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <Card key={item.id} className="glass-panel border-white/5">
              <CardContent className="p-4 flex items-center gap-4">
                <div className="h-16 w-16 rounded-lg bg-black/50 overflow-hidden shrink-0 border border-white/5">
                  {item.imageUrl ? (
                ...
                <div className="flex items-center gap-3">
                  <div className="flex items-center bg-black/40 rounded-lg border border-white/10">
                    <Button variant="ghost" size="icon" className="h-8 w-8 text-white hover:bg-white/10 hover:text-white" onClick={() => updateQuantity(item.id, item.quantity - 1)}>
        ...
        <div className="space-y-6">
          <Card className="glass-panel border-white/5 sticky top-8">
            <CardHeader>
              <CardTitle className="text-white">Order Summary</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-white/80">Pickup Time</label>
